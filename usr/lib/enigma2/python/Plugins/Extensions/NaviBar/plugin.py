...
config.plugins.navibar.placesize = ConfigSelection(default="550", choices = [("0",_("0")),("50",_("50")),("100",_("100")),("150",_("150")),("200",_("200")),("250",_("250")),("300",_("300")),("350",_("350")),("400",_("400")),("450",_("450")),("500",_("500")),("525",_("525")),("550",_("550")),("575",_("575"))])
config.plugins.navibar.fontsize = ConfigSelection(default="28", choices = [("22",_("22")),("24",_("24")),("26",_("26")),("28",_("28"))])
config.plugins.navibar.transparent = ConfigYesNo(default=False)
config.plugins.navibar.blue = ConfigYesNo(default=True)
config.plugins.navibar.which = ConfigSelection(default="wall", choices = [("bar",_("bar")),("wall",_("wall"))])
...
class navibar_config(Screen, ConfigListScreen):
	skin = """
		<screen position="center,center" size="800,400" title="NaviBar Setup v1.1">
			...
			<ePixmap position="650,370" size="35,35" pixmap="~/images/key_info.png" alphatest="blend" />
			<ePixmap position="700,370" size="35,35" pixmap="~/images/key_menu.png" alphatest="blend" />
			<ePixmap position="750,370" size="35,35" pixmap="~/images/bouquet.png" alphatest="blend" />
		</screen>"""	
	...
	def __init__(self, session, plugin_path):
		...
		self.list.append(getConfigListEntry("Plugins sortieren nach:", config.plugins.navibar.hits))
		self.list.append(getConfigListEntry("NaviBar mit Blue Button starten (enigma2 neustart erforderlich!):", config.plugins.navibar.blue))
		self.list.append(getConfigListEntry("NaviBar Text groesse:", config.plugins.navibar.fontsize))
		self.list.append(getConfigListEntry("NaviBar Position:", config.plugins.navibar.placesize))
		self.list.append(getConfigListEntry("NaviBar Transparent:", config.plugins.navibar.transparent))
		self.list.append(getConfigListEntry("NaviBar Style:", config.plugins.navibar.which))
		self.list.append(getConfigListEntry("Icons Path:", config.plugins.navibar.dlpath))
		...
		{
			"input_date_time":	self.switchtpw,
			"red":				self.del_section,
			"timerAdd":			self.add_section,
			"info":				self.select,
		...
		{
			"showMovies":		self.change_section,
		}, -1)
	...
	def readconfig(self):
		...
		for line in config_read.readlines():
			...
			if ok:
				(name, hits, hide, sort, pw, section) = ok[0]
				self.config_list_select.append((name, hits, hide, sort, pw, section))
		config_read.close()
		
		self.config_list_select.sort(key=lambda x: int(x[3]))
		
		for each in self.config_list_select:
			(name, hits, hide, sort, pw, section) = each
			self.config_list.append(self.show_menu(name, hits, hide, sort, pw, section))
		self["config2"].l.setList(self.config_list)
		self["config2"].l.setItemHeight(22)
		
	def show_menu(self, name, hits, hide, sort, pw, section):
		res = [(name, hits, hide, sort, pw, section)]
		if int(hide) == 0:
				...
				for line in config_read.readlines():
					...
					if ok:
						...
						if list_name.lower() == name.lower():
							config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , hide, sort, pw, next_name))
						else:
							config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , hide, sort, pw, section))
				config_tmp.close()
				config_read.close()
			...
			if item:
				...
				for line in config_read.readlines():
					...
					if ok:
						...
						if list_name.lower() == name.lower():
							if int(hide) == 0:
								config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , "1", sort, pw, section))
							else:
								config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , "0", sort, pw, section))
						else:
							config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , hide, sort, pw, section))
				config_tmp.close()
				config_read.close()
			...
			if not self.selected:
				...
				for each in self.config_list_select:
					...
					if name == self.last_plugin_name:
						...
						config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (self.last_plugin_name, self.last_plugin_hits , self.last_plugin_hide, count_move, self.last_plugin_pw, self.last_plugin_section))
						count_move += 1

					print each[0], count_move
					config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits, hide, count_move, pw, section))
					count_move += 1
		...
		if item:
			...
			for line in config_read.readlines():
				...
				if ok:
					...
					if list_name.lower() == name.lower():
						if int(pw) == 0:
							config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , hide, sort, "1", section))
						else:
							config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , hide, sort, "0", section))
					else:
						config_tmp.write('"%s" "%s" "%s" "%s" "%s" "%s"\n' % (name, hits , hide, sort, pw, section))
			config_tmp.close()
			config_read.close()
...
class navi_wall(Screen):			
	def __init__(self, session, plugin_path, pluginlist, section, sectionlist):
		...
		for each in self.pluginlist_old:
			(plugin_start, name, hits, hide, sort, pw, section) = each
			#if any(item == each[5] for item in self.sectionlist):
			if each[6] == self.section:
				#print each[1]
				self.dump_list.append((plugin_start, name, hits, hide, sort, pw, section))
	...
	def navibar_config(self):
		self.session.openWithCallback(self.reload, navibar_config, plugin_path)
	...
	def starte_plugin(self):
		...
		if plugin_name in self.sectionlist:
			...
			config_read = open(self.plugin_path+"/config" , "rw")
			...
			config_read.close()
			config_tmp.close()
			shutil.move(self.plugin_path+"/config.tmp2", self.plugin_path+"/config")
...
class navi_bar(Screen):			
	...
	def read_config(self):
		...
		for line in config_read.readlines():
			...
			if ok:
				(name, hits, hide, sort, pw, section) = ok[0]
				list.append((name, hits, hide, sort, pw, section))
		config_read.close()
		return list
...
def start(self):
		...
		if not fileExists(plugin_path+"/config"):
			config_first = open(plugin_path+"/config" , "w")
			for each in self.pluginlist:
				config_first.write('"%s" "0" "0" "99" "0" "main"\n' % each[2])
			config_first.close()
			
		elif fileExists(plugin_path+"/config"):
			leer = os.path.getsize(plugin_path+"/config")
			if leer == 0:
				config_first = open(plugin_path+"/config" , "w")
				for each in self.pluginlist:
					config_first.write('"%s" "0" "0" "99" "0" "main"\n' % each[2])
		...
		config_tmp = open(plugin_path+"/config.tmp" , "w")
		self.list_new = []
		for plugin_data in self.pluginlist:
			#print plugin_data
			(plugin_start, none, plugin_name) = plugin_data
			config_read = open(plugin_path+"/config","r")
				...
				if ok:
					...
					if plugin_name.lower() == name.lower():
						...
						if int(hide) == 1:
							...
							self.list_new.append((plugin_start, plugin_name, hits, hide, sort, pw, section))
						break
			...
			config_read.close()
		config_tmp.close()
		shutil.move(plugin_path+"/config.tmp", plugin_path+"/config")
...
def Plugins(path, **kwargs):
	...
	plugin_path = path
	return [
		PluginDescriptor(name="NaviBar", description="Navigate your plugins.", where = [PluginDescriptor.WHERE_PLUGINMENU], fnc=main, icon="plugin.png")
		]